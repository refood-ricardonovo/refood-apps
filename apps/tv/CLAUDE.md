...
- **The screen's identity is núcleo plus ordinal. A location label sits beside it and does not replace it.** The núcleo comes from resolving `company_id` against Odoo; the ordinal is the device's pairing order within that núcleo. Those two are the identity, because neither depends on anyone having remembered to type anything. **Count the ordinal over every row of the núcleo ordered by `criado_em`, revoked ones included** — the obvious implementation counts only the rows on screen, and that is wrong: hiding a revoked device would renumber every screen after it. Stable numbers with gaps are the goal, and a gap is information — someone revoked a screen there. A screen number that changes by itself, because a different screen was revoked, is worse than no number at all.

- **`dispositivos.local` is free text, and it reverses a decision `0002` took. Read why before touching it.** `0002` dropped the `nome` column with two reasons written down: that this app writes only its own technical state and a screen label is not that, and that a name typed on the admin surface could only ever be a guess, because the surface has no identities behind it. Migration `0005` brings a short label back as `local` — "Cozinha", "Sala de convívio" — and neither reason survives contact with what changed:

  - **Who writes it changed.** `nome` was typed by whoever was installing the screen, on the phone, describing a room the person on the other end could not see. `local` is typed at the sede, by one of four people, with the núcleo's screens listed in front of them. The label is no longer a guess; it is read off a list. What has _not_ changed is that the admin has no identities — so the label still carries no authority about **who** wrote it, only about what was seen. Nothing may be inferred from it beyond "somebody at the sede wrote this".
  - **A device's room is technical state, and `0002` said otherwise in order to be able to ban it.** This is not a decision that aged; it was wrong when it was made, and the way it was wrong is the part worth keeping. Where a television physically stands is a property of the television, like `criado_em` or `revogado` — it says nothing about shifts, people, or the work. Calling it _operational_ state was reaching for a category that made the prohibition follow, rather than reading the thing and seeing which category it fell in. The rule "this app writes only its own technical state" was never the problem: a location label has always been inside it. **The lesson is about the move, not about labels** — a rule invoked by first classifying the case to suit the answer will justify anything, and it is worth suspecting whenever a category feels convenient.
  - **What did not change is why it is needed.** With one screen per núcleo, "PT Núcleo Leiria · ecrã 1" was enough. With two it is not, and the ordinal cannot be made enough — it is deliberately an arbitrary sequence number, and making it meaningful would mean renumbering, which is the one thing it must never do.

  **The label is management metadata and never reaches a television.** Not by convention: `autenticarDispositivo` does not select the column, so it is not in `SessaoTv`, so no TV route has it in scope to leak by accident. There is no `SELECT *` in `sessao.ts` and there must not be — `test/ecra.test.ts` goes red if one appears.

  **Two labels alike in one núcleo is the one case where the label stops doing its job**, so it is counted and said. Two "Cozinha" in Leiria tell nobody which screen is which, and somebody revoking by the label revokes the wrong one. It is a warning and not a refusal — there is the morning when a screen is being moved between rooms and the two names collide for a few hours. Counted **on the server**, in `listarDispositivos`, for the same reason the ordinal is: it is a sum over the whole set of rows and the server is the one that has them. Compared without case or accents, because "Cozinha", "cozinha" and "COZINHA" are one place; the comparison errs towards warning, because a spurious warning costs a glance and a missing one costs a revocation on the wrong screen. Revoked screens count on neither side — that screen is off the wall.

  **And the exposure this accepts, said plainly.** Nothing stops someone typing "Cozinha da Dona Maria". The validation caps the length, refuses control characters and collapses whitespace; it cannot police meaning, and pretending otherwise would be worse than admitting it. It is the same trade the volunteers' notes already make in the shared schema, with the same mitigation — the content never reaches a screen anyone can walk past — and it inherits the same open question the root `CLAUDE.md` already names: **retention**. Nothing deletes these labels, and a device row is never deleted.

- **`dispositivos.painel_inicial` is which dashboard a screen opens on, and it is not a lock.** A núcleo with two screens wants one on deliveries and one on collections without somebody pressing the remote every morning. The remote still switches, and that choice is **never written back**: it is a preference for the afternoon, not configuration, and a screen that reboots returns to what the sede chose. Served by `GET /api/ecra`, **a separate route from `/api/nucleo` on purpose** — what `/api/nucleo` returns belongs to the núcleo and is therefore cacheable per núcleo, which is how every Odoo read here works; this belongs to the screen. Put both in one response and the day someone adds a cache to that read, the two televisions in a kitchen open on the same dashboard, which is the exact failure this feature exists to prevent.

  **The column has no `CHECK`, and the reader tolerates what it does not know.** The list of dashboards will grow, and a `CHECK` would make adding one a migration. Validation on write lives in `src/paineis.ts` and the admin route; on read, a television that meets a dashboard its build cannot draw falls back to the default and logs it. That is not laxity — 65 screens update by page reload, each when it happens to reload, so during a rollout the two sides are not on the same version.

  **There are two copies of the dashboard list** — `src/paineis.ts` and `AREAS` in `public/painel.html` — because assets are static and no build step could import the module, the same reason `index.html` repeats the SHA-256 of `auth.ts`. `test/ecra.test.ts` compares them. The admin page has no third copy: the list travels in the `/api/admin/dispositivos` response, so what the page offers and what the server accepts come from one place.

- **Configuring a screen is not scoping it.** `POST /api/admin/dispositivos/configurar` takes two fields from a request body, and neither decides what the screen may read: the dashboard picks among data that token could already see, and the label never leaves the admin. The one route that genuinely takes scope from a request is the pairing approval, with its three fences, and nothing here weakens it. Stated so nobody reasons by analogy: **a field that moved a device between núcleos would not go here.** Moving a screen is revoking it and pairing it again, which is the path that leaves a record on both rows.
...
## No `fetch` without a deadline, on either screen

**A hung `fetch` never rejects.** The network being down is not this case — that rejects, and every path already handles it. This is the weak kitchen wi-fi, the captive portal, the proxy that accepts the connection and then goes quiet: the promise stays alive forever, and so does everything awaiting it. On a television nobody reloads, "forever" means until someone pulls the plug.

So both `pedir` functions — `painel.html` and `index.html` — carry an `AbortController` and a timer, and every caller treats the abort as a failure it already knew how to handle. Three things about it are load-bearing:

- **`AbortController` by hand, not `AbortSignal.timeout`.** The shortcut needs Chrome 103; these screens run on whatever WebView the box shipped with, which on a cheap one can be years behind.
- **The timer is cleared in a `finally`.** Without it every request leaves a live `setTimeout` behind, and these screens stay on for weeks.
- **A timeout and a dead network say different things, and the screen says which.** "Sem ligação à rede" and "O servidor não respondeu a tempo" send whoever is standing in the kitchen to look in different places. The handling is identical; only the sentence differs.

**The startup deadline is a different thing from the transport one, and both exist.** `PRAZO_DO_PEDIDO_MS` stops a hung connection from hanging the rest. The four seconds `escolherAreaInicial` gets is patience, not transport: the screen's configuration is dispensable, and waiting fifteen seconds for it before drawing anything would be paying for something a remote-control keypress fixes. This distinction is what a single timeout constant would have flattened — and flattening it is how the panel nearly shipped a boot path that could leave a blank screen with no clock and no retry.
...
**There is no bare `deploy` script, deliberately — and this app learned it the hard way.** Production lives at the **top level** of `wrangler.jsonc`, so `wrangler deploy` with no `--env` targets production and says nothing about it. `npm run deploy:staging` and `npm run deploy:producao` each carry their own environment, and the bare `deploy` now refuses and names the two. It is the same class of mistake the root `CLAUDE.md` documents for `d1 migrations apply`, it is the treatment `apps/app` already gave itself, and this app went without it until somebody ran the wrong one. Keep the guard: a command whose name does not say where it goes is the whole defect.

**What that costs, and this pointer is the whole mitigation:** `npm run deploy:staging --workspace=tv` no longer describes a complete deployment of this app. The Worker ships from here, the schema it runs against ships from the root, in a separate step that cannot be inferred from anything inside `apps/tv`. That is the price paid for a shared database with one ledger, and it was paid deliberately. Do not let this paragraph rot.
