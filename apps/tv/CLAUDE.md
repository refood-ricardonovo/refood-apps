...
- **Never render full names — first name and initial only.** These screens face a room that anyone can walk through, so the privacy boundary is the screen itself, not the login. **This is about natural persons.** An _entity_ — a food source, a support partner — is not personal data and its name appears; it comes from the ficha's own `name`, never from a `pos.order`'s `client_name` and never from `res.partner`. A support partner's name is cut to **the first two words**, because nothing in `res.support.partner` forces a partner to be an institution — 651 of 1 014 have no type at all, and the field a beneficiary uses to point at the partner who accompanies them says "typically the social worker". Two words identify _Centro Paroquial_ and do not spell anybody's full name.
