...
/*
 * As migrações estão na raiz do monorepo — `apps/tv/test/` → três níveis acima. A base é
 * partilhada com a PWA, portanto o esquema é do monorepo e não desta app.
 *
 * **Isto monta o esquema inteiro, incluindo tabelas que a TV nunca toca, e é deliberado.**
 * Uma base partilhada testada contra metade do esquema é pior do que não a testar: uma migração
 * da PWA que colida com o que a TV lê tem de aparecer aqui, a vermelho, e não em produção. O
 * preço é que uma migração da PWA pode partir os testes desta app — é essa a rede, não um efeito
