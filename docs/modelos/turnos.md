...
precisa de outro critério (ver *Pontos em aberto*).
...
- **Só um subconjunto dos voluntários pode ser gestor de turno.** A lista de escolha do
  `shift_manager_id` são os que têm um `job_id` com `is_manager` — *Voluntário Gestor* e *Voluntário
  Gestor de Turno* (ver [`voluntarios.md`](voluntarios.md)). Nada na base impede que se escreva lá
